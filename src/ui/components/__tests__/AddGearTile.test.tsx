...
describe('AddGearTile', () => {
  it('renders the label', async () => {
    const { getByText } = await render(<AddGearTile label="Set Up Smart Bike" onPress={() => {}} />);
    expect(getByText('Set Up Smart Bike')).toBeTruthy();
  });

  it('fires onPress when pressed', async () => {
    const onPress = jest.fn();
    const { getByText } = await render(<AddGearTile label="Set Up Smart Bike" onPress={onPress} />);
    await fireEvent.press(getByText('Set Up Smart Bike'));
    expect(onPress).toHaveBeenCalledTimes(1);
  });

  it('renders noir label color', async () => {
    const { getByText } = await render(<AddGearTile label="Set Up Smart Bike" onPress={() => {}} scheme="noir" />);
