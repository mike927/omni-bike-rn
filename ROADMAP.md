...
- [x] Wire onboarding pairing CTAs to real pairing flow — `Search for Bike` and `Pair Device` currently just advance the carousel. Likely shape: modal-present `GearSetupScreen` on top of onboarding so dismiss returns to the same step. Decide between (a) modal, (b) deeplink + back, (c) inline mini-pairing UI before implementation.
