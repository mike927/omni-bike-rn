...
- **Code Quality Pass**: Map `ai/workflow/steps.md § 8. Internal Review` Phase A (pre-review code quality pass) to your native `/simplify` tool.
- **Dedicated Reviewer Subagent**: Map `ai/workflow/steps.md § 8. Internal Review` Phase B (deep review) to the `Agent` tool. Use a `code-reviewer` subagent type if one is defined under `.claude/agents/`, otherwise use `general-purpose`. Always pass the review brief (intent, scope boundaries, tradeoffs, plan pointer) in the `prompt` field, and instruct the subagent to execute `/code-review` end-to-end so findings land in `ai/local/reviews/<branch-slug>.md`.
