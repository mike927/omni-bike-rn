...
Review the active feature branch's canonical plan file and record what is already strong, what must change, and what decisions are still missing before implementation continues. This command is the plan-quality gate for the Plan phase in `AGENTS.md § Phases`.
...
Review the plan against the drafting requirements in `ai/workflow/steps.md § 2. Plan` and the artifact conventions in `AGENTS.md § Workflow Artifacts`. Do not apply generic prose quality standards — only check alignment with repo rules.
...
3. **Decision completeness** — the implementer would not need to make product, workflow, or architecture decisions during implementation (per `ai/workflow/steps.md § 2. Plan`, which requires the plan to be specific enough to execute without further design decisions).
...
File-level rules follow `ai/workflow/review-file.md § Format` (append-mode contract, latest-block-wins, never overwrite). If no prior file exists, create it with a single `# Plan Review: <branch-name>` H1 on line 1, then append the first block below it.
