...
3. **Automated Testing**: Essential business logic and infrastructure services MUST be covered by automated tests.
...
4. **Standardization**: Prefer official or widely adopted, stable React Native and Expo libraries over custom implementations.
5. **Extensibility**: Design hardware interfaces (Bike, Watch) using the Adapter pattern, making it easy to swap or add new hardware support without changing the core training logic.
6. **Definition of Done**: Every change session or new feature must conclude with **zero linter errors** (`eslint`), **all unit tests passing** (e.g., `jest`), and **zero TypeScript compiler errors** (`tsc`). Code quality is non-negotiable.
7. **Agent Code Review Workflow**: When the developer agent completes working on a given feature, it MUST delegate code review to a separate agent (acting as code reviewer). The reviewer agent will analyze the changes and leave comments/suggestions. The developer agent will then analyze these suggestions, apply the relevant ones, and safely ignore the non-relevant ones before finalizing the feature branch.
8. **Human Pull Request Review**: If the human user leaves comments on the GitHub Pull Request, they should paste the GitHub PR URL or summarize the feedback back into the chat. The development agent will then check out the feature branch, analyze the human feedback, apply the requested changes, run the CI checks locally (linter, types, tests), and push the updated branch to automatically update the PR.
