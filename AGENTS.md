...
**Native lifecycle checks:** `npm run test:watch-lifecycle` compiles the Watch companion's pure lifecycle model together with the host checks in `native-tests/` and runs them on the Mac. Deliberately outside `ci:gate`, which runs on Linux with no Swift toolchain; `lint-staged` runs it pre-commit for staged `ios/OmniBikeWatch Watch App/*.swift`. Run it by hand after any other change that touches the lifecycle.
...
- **Distance is workout-relative:** `normalizeDistanceStep` in the accumulator rebases the trainer's own odometer on the first reading and again on every counter reset, so `MetricSnapshot.distance` stays the machine's raw counter and is never a distance the rider covered. Every persisted sample stores the normalized total in `sessionDistanceMeters`; exports read that field, and reconstruct the series through the same step for rows written before the column existed, capping every reconstructed value at the session's stored total so a replay can never run past it (`src/services/export/resolveWorkoutDistancePoints.ts`). Never publish `metrics.distance` as a per-second distance.
...
- **Watch lifecycle intent:** the iPhone owns the ride and the Watch owns its `HKWorkoutSession`, so every ride control crosses WatchConnectivity asynchronously and every HealthKit change it triggers settles asynchronously too. The Watch keeps the *desired* state explicitly (`WatchLifecycleModel` in `ios/OmniBikeWatch Watch App/WatchLifecycle.swift`) and reconciles the session against it after every settled transition, instead of letting whichever callback lands last decide. Newest intent wins: a pause or resume that cannot act yet (a transition still settling) records its intent, and the next settled callback reconciles against it; a stop retires a pending start even when no session exists. A recorded intent alone never *creates* a session, so a `start` that finds a session tearing down is dropped, not retried. Commands that carry the iPhone's send stamp are ordered by it rather than by arrival, because the two transports (`sendMessage` when reachable, `transferUserInfo` queued when not) can deliver them out of order; the HealthKit wake carries no stamp of ours and so applies on arrival. The duplicate-transition interlock stays for pause and resume (a redundant `pause()` makes HealthKit fail the whole session) but never gates `end()`: a transition that never settles would otherwise strand a live workout no command could stop. `WorkoutManager` is the only thing that turns a decision into a HealthKit call, and the model stays free of platform imports so `npm run test:watch-lifecycle` can exercise it.
...
- **Ride pause history:** a ride's pause and resume moments are persisted with it (`pauseEvents` on `PersistedTrainingSession`), written by `updateSessionStatus` in the same transaction as the status change so a ride's status and its history can never disagree. Why it paused is irrelevant here: manual, bike-driven and disconnect pauses all stop the 1 Hz clock, so all of them are the same fact. `null` means the history is unknown (a ride recorded before the app kept one) and is **not** the same as `[]`; never fabricate intervals for an unknown history. Apple Health exports the history as HKWorkout pause/resume events, which is what makes HealthKit report the ride's active duration; the workout keeps its real start and end so no sample falls outside it.
