...
- **Advertisement packet does NOT include `0x180D`.** This is the single biggest gotcha for iOS integration on any Garmin model that does broadcast over BLE at all. The broadcast-capable Garmin families expose the standard BLE HR service `0x180D` / characteristic `0x2A37` through GATT *after* connection, but they do **not** include `0x180D` in the advertisement packet itself — the advertisement carries only manufacturer data (Garmin Company ID `0x0087`) and sometimes a local name. iOS `CBCentralManager.scanForPeripherals(withServices:)` filters strictly by advertised service UUIDs, which means passing `[0x180D]` as a scan filter silently drops every such watch from scan results even though it is a valid BLE HR sensor post-connect. Chest straps (e.g. Garmin HRM-Dual) do advertise `0x180D` and are unaffected. Note: this gotcha is orthogonal to the Venu gen 1 negative result documented above — Venu gen 1 firmware 7.80 does not emit a BLE advertisement at all in broadcast mode, so for that specific hardware there is nothing for any scan filter strategy to recover. The advertisement-omits-`0x180D` gotcha is the reason *newer* broadcast-capable Garmin watches still require the no-service-filter scan path; do not cite Venu gen 1 as the positive example here.
