...
- **Command Picker Bridges**: `.codex/skills/<name>/SKILL.md` are the official thin Codex skill bridges that surface repo commands in the `$`/`/` picker. Each bridge's `description` carries the trigger contract; the body is a one-line pointer to the canonical `ai/commands/<name>/COMMAND.md`. Edit the canonical COMMAND.md, not the bridge — the bridge only re-registers the skill for Codex's picker.
